...
const GUIDE_SEEN_KEY = "gm.swap.guideSeen";
...
export function SwapExperience() {
  ...
  function tradeEmber() {
    if (!EMBER_MINT) {
      setQuoteStatus("error");
      setQuoteError("$GM is not live yet. Add the mint after launch to enable direct GM trading.");
      return;
    }
    ...
    if (!emberToken || emberToken.disabled) {
      setQuoteStatus("error");
      setQuoteError("$GM is not available in the token list yet.");
      return;
    }
    ...
    <>
      ...
      <main className={guideOpen ? "home-page blurred" : "home-page"}>
        <section className="hero">
          ...
          <p>{isPrelaunch ? "Best-route Solana swaps powered by Jupiter. GM trading opens after the pump.fun launch." : "Best-route Solana swaps powered by Jupiter across live Solana DEX liquidity."}</p>
        </section>

        <section className="official-token">
          <div className="token-emblem">
            <Image src={EMBER_TOKEN.image} alt="GM SWAP logo" width={74} height={74} />
          </div>
          <div className="official-identity">
            <div>
              ...
              <span>{isPrelaunch ? "$GM launches on pump.fun soon. Mint will be published after launch." : "$GM is live. Data updates from Solana liquidity markets."}</span>
            </div>
          </div>
          ...
          <div className="token-actions">
            <button onClick={tradeEmber} disabled={!EMBER_MINT}>{EMBER_MINT ? "TRADE $GM" : "$GM PENDING"}</button>
        ...
        <section className="swap-shell" ref={swapShellRef}>
          ...
          <dl className="fee-list">
            <div><dt>GM fee</dt><dd>{formatBps(SWAP_FEE_BPS)}</dd></div>
          ...
          <div className="reward-line">$GM holder XP updates after confirmed swaps</div>
          {isPrelaunch && (
            <p className="swap-error">$GM is not live yet. Swaps for other listed tokens are already available.</p>
          )}
      ...
      <footer>
        ...
        <span><a href="/" target="_blank">GM SWAP</a> - Built on <b>Solana</b></span>
      </footer>
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="guide-title">
      <section className="guide-modal">
        ...
        {[
          ["You stay in control", "GM SWAP never sees your recovery phrase. Your wallet previews and signs every transaction."],
          ["Network costs only", "GM SWAP currently charges no platform fee. Solana may reserve about 0.002 SOL when your wallet needs a new token account."],
