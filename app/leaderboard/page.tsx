...
export default async function LeaderboardPage() {
  ...
  const stats = [
    ["VERIFIED SWAPS", String(totalSwaps), leaders.length > 0 ? "Confirmed GM SWAP trades" : "No verified swaps yet"],
      ...
      <main className="dashboard-page">
          ...
          <div>
            ...
            <p>
              {season.isLive && season.end
                ? `${season.label} tracks confirmed GM SWAP transactions and wallet XP. Current season closes ${formatSeasonDate(season.end)} UTC.`
                : "Confirmed GM SWAP trades are already tracked. Season Zero XP starts automatically when the $GM mint and pump.fun link go live."}
            </p>
          </div>
        ...
        <aside className="warning-strip">
          {season.isLive
            ...
            : "Pre-launch swaps are saved as verified activity. Holder-gated XP begins when $GM goes live."}
        </aside>
      </main>
    </SiteShell>
  );
